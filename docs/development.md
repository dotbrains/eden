...
- `build` runs `python -m build` once on Ubuntu in Flox.
- `test-compat` runs `pytest -v -m unit` for Linux/Python 3.12 in Flox,
  macOS/Python 3.13 in Flox, and Windows/Python 3.13 via `actions/setup-python`.
...
at or below 242 lines, test files stay at or below 242 lines, active docs stay
at or below 236 lines, source directories stay at or below 29 tracked files,
and test directories stay at or below 142 tracked files. Historical
