...
`eden init` scaffolds a project from a template. Five templates ship today: `blank` (minimal), `simple-loop` (an iteration-driven worker over a backlog manager), `sequential-reviewer` (implement-then-review per task on a shared sandbox), `parallel-planner` (plan + parallel-execute + merge over the unblocked backlog), and `plan-implement-review` (three sequential agents: planner produces a structured plan, implementer executes it, reviewer audits the diff).
...
A runnable worker that picks open tasks from a backlog manager and processes them one per iteration. Adapted from upstream's `simple-loop` template.
