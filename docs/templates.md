...
A runnable worker that picks open tasks from a backlog manager (GitHub Issues or Beads) and processes them one per iteration. Adapted from upstream's `simple-loop` template.
