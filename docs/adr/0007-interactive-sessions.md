...
- **Only `no_sandbox` is supported initially.** Containerized TTY (`docker exec -t` / `podman exec -t`) requires extending the sandbox handle Protocol with an `interactive_exec` method and is tracked separately — see [`docs/adr/0009-containerized-tty-for-interactive.md`](0009-containerized-tty-for-interactive.md). Other providers raise `InvalidOptions` with a clear hint.
...
- Upstream 0.4.6 (`interactive()` API), 0.4.6 (`noSandbox` provider).
