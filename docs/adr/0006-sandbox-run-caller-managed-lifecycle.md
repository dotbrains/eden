...
Upstream 0.4.4 solved this with `createSandbox()` returning a `Sandbox` whose `.run()` method reuses the worktree and container. Eden needed the equivalent.
...
- Upstream 0.4.4 (`createSandbox`), 0.4.6 (interactive equivalent), 0.4.x (sequential-reviewer template).
