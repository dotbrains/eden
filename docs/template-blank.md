...
Detailed reference for the `blank` template. See [Templates](templates.md) for the other project scaffolds.
...
`{{SOURCE_BRANCH}}` and `{{TARGET_BRANCH}}` substitutions, and `` !`cmd` `` shell-block expansion, are available. See [Prompts](prompts.md).
...
These keys are read by the agent CLIs themselves, not by Eden. See [Configuration](configuration.md) for the env vars Eden itself reads.
