...
def test_real_agent_readme_uses_eden_image_builder() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")
    assert "eden docker build-image" in readme
    assert "docker build -t eden:$(basename $(pwd)) -f .eden/Dockerfile ." not in readme
